...
{
    std::string response = "";
    if (clients[client_fd].channels.empty())
    {
        std::cout << "empty channels" << std::endl;
        return response;
    }

    for (std::vector<std::string>::const_iterator it = channels[channel].operators.begin(); it != channels[channel].operators.end(); ++it)
    {
        if (it->compare(clients[client_fd].username) == 0)
        {
            response = clients[client_fd].username;
            break;
        }
    }
...
void execute_topic(int client_fd, std::vector<std::string> &params, std::map<int, Client> &clients, std::map<std::string, Channel> &channels)
{
        if (params[0].empty())
        {
            std::string err_msg = ":monserver 461 " + clients[client_fd].username + " TOPIC :Not enough parameters\r\n";
        ...
        if (exist == 0)
        {
            std::string err_msg = ":monserver 403 " + clients[client_fd].username + " TOPIC :No such channel\r\n";
            send(client_fd, err_msg.c_str(), err_msg.length(), 0);
            return;
        }
        ...
        {
            std::string err_msg = ":monserver 442 " + clients[client_fd].username + " " + channel + " :You're not on that channel\r\n";
            send(client_fd, err_msg.c_str(), err_msg.length(), 0);
            return;
        }

        if (params.size() == 1 && channels[channel].topic.empty())
        {
            std::string topic_msg = ":monserver 331 " + clients[client_fd].username + " " + channel + " :No topic is set\r\n";
            send(client_fd, topic_msg.c_str(), topic_msg.length(), 0);
            return;
        }
        if (params.size() == 1 && !channels[channel].topic.empty())
        {
            std::string topic_msg = ":monserver 332 " + clients[client_fd].username + " " + channel + " :" + channels[channel].topic + "\r\n";
            send(client_fd, topic_msg.c_str(), topic_msg.length(), 0);
            return;
        }
        ...
        if (params.size() > 1)
        {
            ...
            if (channels[channel].user_modes["topic_lock"] == 1 && whois_operator(clients, channel, client_fd, channels).compare(clients[client_fd].username) != 0)
            {
                std::string err_msg = ":monserver 482 " + clients[client_fd].username + " TOPIC :You're not channel operator\r\n";
                send(client_fd, err_msg.c_str(), err_msg.length(), 0);
                return;
            }
            ...
            std::string topic_msg = ":" + clients[client_fd].username + "!" + clients[client_fd].realname + " TOPIC " + channel + " :" + topic + "\r\n";
