...
void handle_mode_command(int client_fd, const std::vector<std::string>& params, std::map<std::string, Channel>& channels, std::map<int, Client>& clients) {
    ...
    if (whois_operator(clients, channel_name, client_fd, channels).compare(clients[client_fd].get_username()) != 0 && channels[channel_name].getOperators().size() > 0) {
        ...
        switch (mode) {
            ...
            case 'o':
                ...
                if (adding_mode) {
                    ...
                    broadcast_message_to_channel(response, channel_name, clients);
                } else {
